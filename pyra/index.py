...
class TextIndexer(object):
    ...
    def _add_doc(self, writer, path):
        ...
        writer.add_document(
            path=path, time=modtime, keywords=keywords, summary=summary, content=content
        )
    ...
    def _get_schema(self):
        return Schema(
            ...
            time=STORED,
            keywords=KEYWORD(lowercase=True, commas=True, stored=True),
    ...
    def clean_index(self):
        ...
        ix = index.create_in(self._index_location, schema=self._get_schema())
        writer = ix.writer()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        "pyra-index",
        description="Whoosh text indexer",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument(
        "-i",
        "--index",
        help="Index database location",
        default=os.path.expanduser("~/.local/share/pyra-index"),
    )
    parser.add_argument(
        "-d",
        "--documents",
        help="Folder of text documents to index",
        default=os.path.expanduser("~/ownCloud/Documents"),
    )
    parser.add_argument(
        "-s", "--summary", help="Number of summary sentences", default=3, type=int
    )
    parser.add_argument("-c", "--clear", help="Clear and reindex", action="store_true")
